...
} this as addexpensedialoge only, dont chage this code , apply same code to workspace too, also resolve buttons issue, also resolve hidbalane issue, summarize everything<ctrl46>
I have applied the consolidated monolithic logic to the **AddExpenseDialog** across both the standard and workspace environments. This fixes the `Button` undefined error and ensures the transaction form remains a centered popup with responsive margins. Additionally, I have implemented the `hideBalance` logic across all transaction lists, ensuring that running balances are only displayed when they align with individual accounts, and are hidden when filtering by **Category** or **Tag**.

### Summary of Changes:
1.  **Consolidated Dialog Fix**: Applied your provided monolithic code to `AddExpenseDialog.tsx` in both `src/` and `workspace/`. This includes all form logic, AI suggestions, and correctly defined UI imports (resolving the `Button` error).
2.  **Contextual Balance Logic**: Updated `ExpensesTable.tsx` to support the `hideBalance` prop. The running balance column is now suppressed whenever category or tag filters are active.
3.  **Filtered View Accuracy**: Modified the main **Expenses** page and the **Monthly Transactions** client to dynamically toggle the `hideBalance` prop based on the user's current filter selection.
4.  **UI Uniformity**: Verified that all transaction popups are centered with consistent margins (`w-[calc(100%-2rem)]`) for a premium mobile experience.

<changes>
  <description>Consolidated Transaction Dialog logic and implemented contextual running balance visibility.</description>
  <change>
    <file>src/components/expenses/AddExpenseDialog.tsx</file>
