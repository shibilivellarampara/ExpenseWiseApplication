        ...
        <div className="space-y-4">
            <div className="relative group">
                ...
                <Input
                    ...
                    className="pl-11 pr-11 h-12 rounded-[16px] bg-card border-none shadow-sm focus-visible:ring-1 focus-visible:ring-primary/20 text-base"
                />
                 {filters.searchQuery && (
                    <Button
                        variant="ghost"
            ...
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 pb-1">
                <Popover>
                    <PopoverTrigger asChild>
                         <Button variant="outline" size="sm" className="h-9 rounded-full px-4 shrink-0 border-muted-foreground/20 text-[11px] font-bold uppercase tracking-widest hover:bg-card">
                            <ListFilter className="mr-2 h-3.5 w-3.5" />
                            Filters
                            {activeFilterCount > 0 && 
                                <span className="ml-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[9px] text-primary-foreground">{activeFilterCount}</span>
                            }
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-80 p-5 rounded-[24px] border-none shadow-[0_20px_50px_rgba(0,0,0,0.2)] bg-card" align="start">
                ...
                {!disableDateFilter && (
                    <Badge variant={filters.dateRange.from ? "secondary" : "outline"} className="h-9 rounded-full px-4 cursor-default whitespace-nowrap bg-transparent border-muted-foreground/20 text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
                ...
                <div className="flex gap-2">
                    {filters.categories.map(id => {
                        ...
                        return item ? (
                            <Badge key={id} variant="secondary" className="h-9 rounded-full pl-3 pr-1 py-0 bg-primary/5 text-primary border-primary/10 text-[11px] font-bold uppercase tracking-widest flex items-center gap-1.5">
                                {item.name}
