...
export const renderIcon = (iconName: string | undefined, className?: string) => {
  if (!iconName) return <LucideIcons.Pilcrow className={cn("h-4 w-4", className)} />;
  
  // Use specific key access to allow for tree-shaking and better Turbopack performance
  const IconComponent = (LucideIcons as any)[iconName];
  
  // Removed hardcoded text-muted-foreground so icons inherit parent text color (e.g. in badges)
